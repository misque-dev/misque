...
export default function ArticleLayout() {
	...
	return (
		<div className={cn("fixed start-0 top-14")}>
			<aside
				className={cn(
					"navbar:transition-all",
					"border-r border-border navbar:flex hidden navbar:w-[268px] lg:w-[286px]! overflow-y-auto h-[calc(100dvh-56px)] pb-2 flex-col justify-between w-[var(--fd-sidebar-width)]",
				)}
			>
				<div>
					<SidebarTab group={group} setGroup={setGroup} />
					<button
						className="flex w-full items-center gap-2 px-5 py-2.5 border-b border-border text-muted-foreground dark:bg-zinc-950 dark:border-t-zinc-900/30 dark:border-t"
						onClick={() => {
							setOpenSearch(true);
						}}
					>
					...
					>
						<div className="flex flex-col">
							{cts.map((item, index) => (
								<div key={item.title}>
									<button
										className="border-b border-border w-full hover:underline text-sm px-5 py-2.5 text-left flex items-center gap-2"
										onClick={() => {
...
const tabs = [
	{
		...
		title: "Docs",
		description: "get started and packages",
	},
	{
		value: "examples",
		icon: (
